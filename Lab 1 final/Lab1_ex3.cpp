...
void verification1( int len , int lung, string lettter[]) {
    int i, j , k, m;
    string a;
    int l = len + 1; //4     len = 3 
    for (i = 0; i < len; i++)
    {
        for (k = 0; k < lung; k++)
        {
            ...
            for (j = 0; j < l; j++)
            {
                ...
                if ((a.length() == 1) && (a != "0"))
                {
                    if ((a == lettter[k]) && (adj[i][len] == lettter[lung-1]))
                    {
                        i = j;
                        cout << "Acept"<< endl;
                        goto jump1;
                ...
                if ((a.length() > 1) && (a != "0"))
                {
                    int lung_imbin = a.length();
                    string imb[20];
                    for (m = 0; m < lung_imbin; m++)
                    {
                        imb[m] = a[m];
                    }
                    for (m = 0; m < lung_imbin; m++)
                    {
                        if ((imb[m] == lettter[k]) && (adj[i][len] == lettter[lung-1]))
                        {
                            i = j;
                            goto jump1;
                        }
...
int main()
{
    ...
    verification1 (length, lungime, str);
